...
function upsertEnvVarLine(lines: string[], key: string, value: string) {
  const target = `${key}=`
...
async function syncFrontendEnvLocal(publishableToken: string) {
  const frontendEnvPath = path.resolve(__dirname, "../../../frontend/.env.local")
  const desiredBackendUrl = String(process.env.NEXT_PUBLIC_BACKEND_URL || "").trim() || "http://localhost:9000"

  let content = ""
  try {
    content = await fs.readFile(frontendEnvPath, "utf8")
  } catch {
    content = ""
  }
  ...
  upsertEnvVarLine(lines, "NEXT_PUBLIC_BACKEND_URL", desiredBackendUrl)
  upsertEnvVarLine(lines, "NEXT_PUBLIC_PUBLISHABLE_API_KEY", publishableToken)

  while (lines.length > 0 && !lines[lines.length - 1]?.trim()) {
    lines.pop()
  }

  const finalContent = `${lines.join(newline)}${newline}`
  await fs.writeFile(frontendEnvPath, finalContent, "utf8")
}
...
export async function runSeed() {
  ...
  if (publishableKey?.token) {
    try {
      await syncFrontendEnvLocal(publishableKey.token)
      logger.info("Synced frontend/.env.local with NEXT_PUBLIC_PUBLISHABLE_API_KEY.")
    } catch (error) {
      logger.warn(`Could not sync frontend/.env.local: ${String(error)}`)
    }
